...
docker --config ~/.docker login https://index.docker.io/v1 -u $DOCKERHUB_USERNAME -p $DOCKERHUB_PASSWORD
docker --config ~/.docker login harbor.operators.management.eks.us-west-2.aws.smarsh.cloud -u $HARBOR_USERNAME -p $HARBOR_PASSWORD

earthly bootstrap --with-autocomplete
cd source-repo
earthly --push -P --secret SNYK_TOKEN --secret HARBOR_USERNAME --secret HARBOR_PASSWORD   +build-scan-and-publish
earthly --push -P  --secret HARBOR_USERNAME --secret HARBOR_PASSWORD --secret VAULT_TOKEN --secret VAULT_ADDR --secret VAULT_NAMESPACE +cosign-image